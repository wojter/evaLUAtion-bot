...
function trojanBot1whatTo(agent, actorKnowledge, time)
    ...
    if (friends:size() > 0) then
        if (friends:size() > 1) then
            if ((friends:at(1):getPosition() - actorKnowledge:getPosition()):length() < 20) then
            end
        end
        -- ------get Health------
        ...
        -- ------get Ammo------
        if (actorKnowledge:getAmmo(Enumerations.RocketLuncher) == 0 and actorKnowledge:getAmmo(Enumerations.Railgun) ==
            0 and actorKnowledge:getAmmo(Enumerations.Shotgun) == 0 and actorKnowledge:getAmmo(Enumerations.Chaingun) ==
            0) then
            triggToMove = -1
        ...
        elseif (whichShoot ~= -1) then
            ...
            if (whichShootDist < 140) then
                ...
                if (actorKnowledge:getWeaponType() ~= Enumerations.Shotgun and
                    actorKnowledge:getAmmo(Enumerations.Shotgun) > 0) then
                    agent:selectWeapon(Enumerations.Shotgun)
                else
            ...
            elseif (whichShootDist < 350) then
                ...
                if (actorKnowledge:getWeaponType() ~= Enumerations.Railgun and
                    actorKnowledge:getAmmo(Enumerations.Railgun) > 0) then
                    agent:selectWeapon(Enumerations.Railgun)
                else
                    agent:shootAt(enemies:at(whichShoot))
                end
